...
> **Note:** The backend runs on Render's free tier — first request after inactivity may take ~30 seconds to wake up. The simulation engine generates live synthetic data continuously once active.
...
$$
\frac{dM}{dt} = \text{median}\left\lbrace \frac{M_j - M_i}{t_j - t_i} \;\middle|\; 1 \le i < j \le N \right\rbrace
$$
